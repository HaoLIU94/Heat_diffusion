...
//
    ...
    for ( i = 1; i < x_num - 1; i++ )
    {
      ...
      if (choice == 1)
  		b[i] = u[i+(j-1)*x_num] + t_delt * fvec[i]/(cuivre.den*cuivre.c);
  	  if (choice == 2)
  		b[i] = u[i+(j-1)*x_num] + t_delt * fvec[i]/(fer.den*fer.c);
  	  if (choice == 3)
  		b[i] = u[i+(j-1)*x_num] + t_delt * fvec[i]/(verre.den*cuivre.c);
  	  if (choice == 4)
  		b[i] = u[i+(j-1)*x_num] + t_delt * fvec[i]/(ploystyrene.den*ploystyrene.c);
    }
